...
$DocumentsRequired = str_replace("'","\'",$_POST['DocumentsRequired']);	
	...
	{
		$sqlupd=mysqli_query($con,"UPDATE jobs SET CompanyId='$_POST[CompanyName]',JobTitle='$JobTitle',JobLocation='$_POST[JobLocation]',JobResponsibility='$JobResponsibility',Eligibility='$chkvalues',SelectionProcess='$SelectionProcess',Compensation='$Compensation',InterviewDate='$_POST[InterviewDate]',RegistrationTime='$_POST[RegistrationDate]',Venue='$Venue',DocReq='$DocumentsRequired',Status='$_POST[Status]' WHERE JobId='$_GET[editid]'");
	
				if(mysqli_affected_rows($con) == 1)
				{
					$qresulti =  1;
					echo  "<font color='green'><h3><strong>Record updated successfully...</strong></h3></font>";
				}
				else
				{
					$qresulti =  1;
					echo  "<font color='red'><strong><h3>No records found to update...</h3></strong></font>";	
				}
	}
	else
	{		
	$sqlins=mysqli_query($con,"INSERT into jobs(CompanyId,JobTitle,JobLocation,JobResponsibility,Eligibility,SelectionProcess,Compensation,InterviewDate,RegistrationTime,Venue,DocReq,Status)VALUES('$_POST[CompanyName]','$JobTitle','$_POST[JobLocation]','$JobResponsibility','$chkvalues','$SelectionProcess','$Compensation','$_POST[InterviewDate]','$_POST[RegistrationDate]','$Venue','$DocumentsRequired','$_POST[Status]')");

			if(!$sqlins)
			{
				echo "<font color='red'><strong><h3>Failed to insert record in database...<h3></strong></font>";
			}
			else
			{
				echo "<font color='green'><strong><h3>Record inserted successfully...</h3></strong></font>";
			}
}
}
}
if(isset($_GET['editid']))
{
	$sqlst = mysqli_query($con,"SELECT * FROM jobs WHERE JobId='$_GET[editid]'");
...
?>             
  ...
  ?>
          ...
          <div class="col-xs-12 col-sm-6 col-lg-8">
            ...
            <p>
<form name="jobform" action="" method="post"  onsubmit="return validate()" enctype="multipart/form-data">
...
<td>
<?php
$sqlselcourse = mysqli_query($con,"select * from course");
?>
